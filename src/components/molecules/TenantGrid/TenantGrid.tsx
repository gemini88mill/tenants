...
const TenantItem = ({ tenant }: { tenant: Tenant }) => {
  const { actions } = useDrawerInputContext();
      ...
      <Grid container spacing={2} alignItems={"baseline"} maxWidth={1}>
        ...
        <Grid item xs={5} textAlign={"start"}>
          ...
          <Stack direction={"column"} spacing={2} alignItems={"baseline"}>
            <p>
              {tenant.addresses?.street_address}, {tenant.addresses?.city},{" "}
              {tenant.addresses?.state_province}
            </p>
          </Stack>
        </Grid>
        <Grid
        ...
        >
          <Grid item xs={8} textAlign={"start"}>
            <small>Lease</small>
            <p>
              {toDate(tenant.lease_start_date ?? "")} -{" "}
              {toDate(tenant.lease_end_date ?? "")}
            </p>
          </Grid>
          <Grid item xs={4}>
            <Button
